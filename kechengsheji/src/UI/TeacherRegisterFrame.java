 ...
 *
 * 文件名称： TeacherRegisterFrame.java
 * 文件标识：无
 * 内容摘要：教师端注册界面，用于向数据库存储教师信息
 * 其它说明：
 * 当前版本： V1.0
 * 作   者：李世赟
 * 完成日期： 20241130
...
public class TeacherRegisterFrame extends JFrame {
    ...
    /*********************************************************
     * 功能描述：构造函数，用于初始化教师注册界面框架
     * 输入参数：无
     * 返回值：无
     * 其它说明：设置框架标题、关闭操作、大小和位置，并使其可见。创建面板，添加账号、密码、工号、姓名、性别、年龄、职称、手机号标签及文本框与注册、返回按钮，设置字体与按钮样式，布局组件，为按钮绑定动作监听器以处理注册和返回逻辑，构建教师注册交互界面布局。
     ************************************************************/
 ...
 * 返回值：无
 * 其它说明：获取各文本框输入，校验非空后调用 isUsernameExists 查用户名是否存在。若不存在，调用 registerTeacher 注册并提示成功；否则提示已存在。遇 SQL 或类加载异常则打印堆栈跟踪排查，确保注册流程准确可靠，避免用户名冲突。
 ************************************************************/
 ...
 * 返回值：无
 * 其它说明：点击时关闭当前注册界面，实现返回功能，优化用户操作流程与界面交互体验，提供操作灵活性与便捷性，便于用户导航。
 ************************************************************/
     ...
     * 其它说明：获取数据库连接，构建查询语句，设置用户名参数查询。若结果集有下一条记录则存在，关闭资源返回 true；否则返回 false，为注册时用户名唯一性校验提供数据依据，维护系统用户数据规范性。
     ************************************************************/
     ...
     * - tphone：手机号
     * 返回值：无
     * 其它说明：获取连接，先后向 Users 与 Teacher 表插入账号与教师信息。构建 SQL 设参执行更新，成功则提示注册成功；遇 SQL 异常提示错误打印堆栈跟踪，保障注册功能稳定运行及数据完整性，完善用户注册流程。
     ************************************************************/
